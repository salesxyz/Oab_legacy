...
describe('Auth', () => {
    ...
    it('faz login com credenciais corretas', async () => {
        const res = await (0, supertest_1.default)(app_1.app).post('/auth/login').send({ email: user.email, password: user.password });
        expect(res.status).toBe(200);
        expect(res.body.accessToken).toBeDefined();
    });
    it('rejeita login com senha incorreta com mensagem genérica', async () => {
        const res = await (0, supertest_1.default)(app_1.app).post('/auth/login').send({ email: user.email, password: 'senhaErrada1' });
        expect(res.status).toBe(401);
        expect(res.body.code).toBe('INVALID_CREDENTIALS');
    });
    it('rejeita login de usuário inexistente com a MESMA mensagem genérica (anti-enumeração)', async () => {
        const res = await (0, supertest_1.default)(app_1.app).post('/auth/login').send({ email: 'naoexiste@teste.com', password: 'qualquerSenha1' });
        expect(res.status).toBe(401);
        expect(res.body.code).toBe('INVALID_CREDENTIALS');
    });
    it('bloqueia acesso a rota protegida sem token', async () => {
        const res = await (0, supertest_1.default)(app_1.app).get('/profile');
        expect(res.status).toBe(401);
    });
    it('permite acesso a rota protegida com token válido', async () => {
        ...
        const res = await (0, supertest_1.default)(app_1.app).get('/profile').set('Authorization', `Bearer ${login.body.accessToken}`);
        expect(res.status).toBe(200);
    ...
    it('fluxo de recuperação de senha responde de forma genérica mesmo para email inexistente', async () => {
        const res = await (0, supertest_1.default)(app_1.app).post('/auth/forgot-password').send({ email: 'ninguem@teste.com' });
        expect(res.status).toBe(200);
    });
    it('rejeita código de verificação inválido', async () => {
        await (0, supertest_1.default)(app_1.app).post('/auth/forgot-password').send({ email: user.email });
        const res = await (0, supertest_1.default)(app_1.app).post('/auth/verify-code').send({ email: user.email, code: '000000' });
