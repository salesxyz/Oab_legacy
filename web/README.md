...
- **`src/services/tokenStore.ts`** — o access token vive **só em memória** (nunca em `localStorage`/`sessionStorage`). O refresh token é persistido porque o backend o devolve no corpo da resposta JSON, não como cookie `httpOnly` — então a única forma de sobreviver a um recarregamento de página é em storage acessível por JS. **Isso é uma concessão de segurança conhecida e documentada**, não um descuido: o endurecimento recomendado para produção é migrar o backend para emitir o refresh token via cookie `httpOnly + Secure + SameSite=Strict`, tirando-o completamente do alcance de JavaScript (e portanto de XSS). "Lembrar acesso" desmarcado usa `sessionStorage` (some ao fechar a aba); marcado usa `localStorage`.
...
- **`src/contexts/AuthContext.tsx`** — ao carregar a aplicação, tenta restaurar a sessão silenciosamente a partir do refresh token salvo (chamando `/auth/refresh`), sem forçar novo login a cada recarregamento de página.
